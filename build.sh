...
FINAL_KERNEL_ZIP=SemloheyPerf-$(date '+%Y%m%d-%H%M').zip ## INI NAMA KERNEL zip NYA
export PATH="$HOME/los/clang/bin:$PATH"
export ARCH=arm64
export SUBARCH=arm64
export KBUILD_BUILD_USER=Djancox
export KBUILD_BUILD_HOST=Semox
export KBUILD_COMPILER_STRING="$($HOME/los/clang/bin/clang --version | head -n 1 | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g' -e 's/[[:space:]]*$//')"

git clone --depth=1 -b main https://gitea.com/LineVall/AnyKernel3 AnyKernel3

if ! [ -d "$HOME/clang" ]; then
echo "Cosmic clang not found! Cloning..."
if ! git clone -q --depth=1 https://gitea.com/LineVall/Clang.git -b zyc20 ~/los/clang; then ## ini Clang nya tools untuk membangun/compile kernel nya (tidak semua kernel mendukung clang)
...
curl -T $FINAL_KERNEL_ZIP -u:f0af46bc-25f3-4f8f-95e8-c3bb2647931a https://pixeldrain.com/api/file/
else
